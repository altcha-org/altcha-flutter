// altcha_pbkdf2.cpp
//
// PBKDF2-HMAC-SHA256/SHA512, single JNI call per key derivation.
//
// SHA-256 tiers (chosen at runtime):
//   1. ARM SHA-2 crypto extensions (ARMv8+crypto) — hardware accelerated
//   2. Scalar C++ fallback — all other CPUs (x86 emulator, old ARM)
//
// SHA-512 always uses scalar C++ (hardware SHA-512 requires ARMv8.2, less universal).
...
#define LOG_TAG "AltchaWidget"
#define LOGI(...) __android_log_print(ANDROID_LOG_INFO, LOG_TAG, __VA_ARGS__)

// ---------------------------------------------------------------------------
// CPU feature detection (ARM64 only)
// ---------------------------------------------------------------------------
...
#ifdef __aarch64__
    g_has_sha2 = (getauxval(AT_HWCAP) & HWCAP_SHA2) != 0;
    LOGI("native: aarch64 sha2=%d", (int)g_has_sha2);
#else
    LOGI("native: non-arm64, scalar SHA only");
#endif
}

// ---------------------------------------------------------------------------
// Portable helpers (shared by scalar & ARM paths)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// SHA-256 constants (16-byte aligned for NEON vld1q_u32)
// ---------------------------------------------------------------------------
...
// Compiles with sha2 target even when the global -march doesn't include it.
...
static void sha256_compress_hw(uint32_t h[8], const uint8_t blk[64]) {
    ...
    // Load 512-bit message block, byte-swap each 32-bit word to big-endian
...
#define SHA256_SCH(m0, m1, m2, m3) \
    ...
    // Rounds 0-3
    SHA256_RNDS4(MSG0, 0)
    // Rounds 4-7: compute w[16..19] for use at rounds 16-19
    ...
    // Rounds 48-51 (last schedule update: w[60..63])
    SHA256_RNDS4(MSG0, 48) SHA256_SCH(MSG3, MSG0, MSG1, MSG2)
    // Rounds 52-63: no more schedule needed
...
// Dispatch: chosen once at runtime by detect_cpu()
static inline void sha256_compress(uint32_t h[8], const uint8_t b[64]) {
#ifdef __aarch64__
    if (g_has_sha2) { sha256_compress_hw(h, b); return; }
#endif
    sha256_compress_scalar(h, b);
}

// ---------------------------------------------------------------------------
// SHA-256 context (Init / Update / Final)
// ---------------------------------------------------------------------------
...
static void sha256_final(Sha256Ctx* c, uint8_t out[32]) {
    ...
    uint8_t p = 0x80;
    sha256_update(c, &p, 1);
    // Pad to 56 bytes in current (or next) block — bulk write, not byte-by-byte.
...
// ---------------------------------------------------------------------------
// SHA-512 (scalar only — ARMv8.2 SHA-512 extensions not universally available)
// ---------------------------------------------------------------------------
...
static void sha512_final(Sha512Ctx* c, uint8_t out[64]) {
    uint64_t bits=c->len*8; uint8_t p=0x80;
    sha512_update(c,&p,1); p=0;
    while(c->fill!=112) sha512_update(c,&p,1);
...
//
// For PBKDF2 the key is constant across all iterations.  Precomputing the
// SHA state after absorbing ipad (64 bytes) and opad (64 bytes) once saves
// 2 SHA-256 compressions per inner-loop iteration — a 2x speedup.
// ---------------------------------------------------------------------------
...
static void hmac_sha256_compute(const HmacSha256Ctx* ctx,
    ...
    Sha256Ctx ic = ctx->ipad;  // clone precomputed ipad state
    sha256_update(&ic,m,mlen);
    uint8_t ih[32]; sha256_final(&ic,ih);
    Sha256Ctx oc = ctx->opad;  // clone precomputed opad state
...
static void hmac_sha512_compute(const HmacSha512Ctx* ctx,
    ...
    Sha512Ctx ic = ctx->ipad;
    sha512_update(&ic,m,mlen);
    uint8_t ih[64]; sha512_final(&ic,ih);
    Sha512Ctx oc = ctx->opad;
...
static void pbkdf2(const uint8_t* pw, size_t pwlen,
                   ...
                   uint8_t* out) {
    const int dlen = (hashId == 512) ? 64 : 32;
    int numBlocks  = (keyLength + dlen - 1) / dlen;

    // Precompute HMAC key schedule once — reused across all blocks & iterations.
    HmacSha256Ctx hmac256; HmacSha512Ctx hmac512;
...
Java_org_altcha_altcha_1widget_AltchaWidgetPlugin_nativePbkdf2(
        JNIEnv* env, jobject,
        jbyteArray jPassword, jbyteArray jSalt,
        jint iterations, jint keyLength, jint hashId) {

    jsize pwlen = env->GetArrayLength(jPassword);
    jsize slen  = env->GetArrayLength(jSalt);
    jbyte* pw   = env->GetByteArrayElements(jPassword, nullptr);
    jbyte* salt = env->GetByteArrayElements(jSalt,     nullptr);

    jbyteArray jOut = env->NewByteArray(keyLength);
    jbyte* out = env->GetByteArrayElements(jOut, nullptr);

    pbkdf2(reinterpret_cast<const uint8_t*>(pw),   pwlen,
           reinterpret_cast<const uint8_t*>(salt),  slen,
           iterations, keyLength, hashId,
           reinterpret_cast<uint8_t*>(out));

    env->ReleaseByteArrayElements(jPassword, pw,   JNI_ABORT);
    env->ReleaseByteArrayElements(jSalt,     salt, JNI_ABORT);
    env->ReleaseByteArrayElements(jOut,      out,  0);
    return jOut;
}
